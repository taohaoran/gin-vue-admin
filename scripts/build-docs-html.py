#!/usr/bin/env python3
"""build-docs-html.py — 将目录下 Markdown 文档渲染为统一样式的 HTML 文档页（GitHub Pages 部署时美化用）。

解决痛点：GitHub Pages 纯静态托管 .md 时浏览器直接显示源码、阅读体验差。
本脚本在部署前把 .md 渲染为带样式的文档页，并保持文档间相对链接跳转与交互图联动。

用法:
  python3 build-docs-html.py <docs_dir> [--site-name "站点名"] [--group-label "core=核心|ai=智能"]
依赖: pip install markdown

行为:
  - 递归渲染 docs_dir 下所有 .md → 同目录同名 .html（.md 保留为源）
  - 文档间相对链接 .md → .html 自动改写
  - 指向 .html 的图片引用（如 archify 交互图）改写为「打开交互式图」按钮，新标签打开
  - 每页顶部生成「← 返回站点首页」面包屑与所属板块标签（按一级子目录分组）
"""
import argparse
import re
import sys
...
def main():
    ap = argparse.ArgumentParser(description="Markdown 文档渲染为带样式 HTML（GitHub Pages 部署时美化）")
    ap.add_argument("docs_dir", type=Path, help="文档目录（默认当前目录）", nargs="?", default=Path("."))
    ap.add_argument("--site-name", default="架构文档", help="站点名（用于面包屑/页脚/标题）")
    ...
    print(f"done: {len(mds)} markdown files rendered")
    return 0
